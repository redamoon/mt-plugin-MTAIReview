...
1. プラグインディレクトリを `plugins/MTAIReview/` に配置
2. 静的ファイル（JavaScript）を `mt-static/plugins/MTAIReview/` に配置
   - `mt-static/plugins/MTAIReview/ai_review.js` を配置
3. Movable Typeの管理画面でプラグインを有効化
4. プラグイン設定でGemini APIキーを設定
...
### プラグインディレクトリ（`plugins/MTAIReview/`）
...
│   └── MT/
│       └── Plugin/
│           └── MTAIReview.pm  # メインロジック
├── tmpl/
│   ├── ai_review.tmpl        # 設定画面テンプレート
│   └── ai_review_script.tmpl # スクリプトテンプレート
└── README.md                 # このファイル
```

### 静的ファイルディレクトリ（MTコアの`mt-static/plugins/MTAIReview/`）

```
mt-static/plugins/MTAIReview/
└── ai_review.js  # フロントエンドJavaScript
```

**注意**: `mt-static`はMTコア側のディレクトリです。プラグインの静的ファイルはMTコアの`mt-static/plugins/MTAIReview/`に配置してください。
